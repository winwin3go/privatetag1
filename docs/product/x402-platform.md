...
During local development, st-idp exposes `/whoami` returning a stub `AuthContext` defined in `pkg/x402-core`, and audit-core exposes `POST /events` accepting `AuditEvent` payloads defined in `pkg/pt-domain`. Data-plane Workers (pt-photo, media-core) emit PHOTO_* audit events while continuing to function even if the stubs are offline (best-effort delivery).
